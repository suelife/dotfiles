...
Windows 上本機程序連容器內 DB，一個請求跑 130 秒——我第一次的結論是
「Docker 跨界就是慢，只能重建容器」。**錯的。**

實測：同一個 DB，`localhost` 三次新連線 **390 秒**，`127.0.0.1` **0.07 秒**——
差 5500 倍。Windows 先把 `localhost` 解析成 IPv6 `::1`，而 Docker 綁的是純 IPv4，
每條連線都在等 IPv6 逾時。連線池每請求 pre_ping，就變成每請求 130 秒。

**做法**：碰到「這個環境就是慢」，先量兩個變體再下結論。逾時型的鈍化
（整齊的 30／60／130 秒）幾乎都是 DNS 或連線 fallback，不是頻寬。
**「只能繞過」這種結論本身就該被懷疑。**
